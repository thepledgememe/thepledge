...
const PledgeWidget: React.FC = () => {
  const {
    pledgedAmount,
    totalPledge,
    pledgeAvailableToSell,
    pledgeWindow,
    isPledgeBroken,
    availableToPLedge,
  } = useAppContext();
    ...
    <div className={styles.container}>
      {isPledgeBroken && (
        <p style={{ color: "red", fontSize: 30 }}>
          YOU BROKE YOUR PLEDGE GO AWAY!!!
        </p>
      )}
      <div className={styles.summary}>
        <h1 className={styles.title}>$PLEDGE BALANCE</h1>
        <h1 className={styles.amount}>
          {totalPledge?.dp(0).toNumber().toLocaleString()}
        </h1>
      </div>
      <div className={styles.details}>
        <div className={styles.section}>
          ...
          <p className={styles.value}>
            {(pledgedAmount?.dp(0).toNumber() || 0).toLocaleString()}
          </p>
        </div>
        <div className={styles.section}>
          ...
          <div>
            <button
              ...
              onClick={togglePopup}
              disabled={
                availableToPLedge.isZero() ||
                availableToPLedge.isLessThan(1) ||
                isPledgeBroken
              }
            >
              <span
              ...
              >
                {(availableToPLedge.dp(0).toNumber() || 0).toLocaleString()}
              </span>
        ...
        <div className={styles.section}>
          ...
          <p className={styles.value}>
            {pledgeAvailableToSell?.dp(0).toNumber().toLocaleString()}
          </p>
        </div>
        <div className={styles.section}>
          <div className={styles.pledgePrice}>
            <strong className={styles.label}>NEW WINDOW OPENS IN</strong>
          </div>
          <p className={styles.value}>{left}</p>
        </div>
      </div>
      <PledgePopup isOpen={isPopupOpen} onClose={togglePopup} />
    </div>
  );
};
